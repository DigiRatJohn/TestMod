...
public class BlinkingTileEntity extends TileEntity implements ITickable {
	...
	// We only calculate this value on the client so we don;t care about the server side and we also don;t care about saving this value.
	// That's why we have no readFromNBT() and writeToNBT().
	private boolean lit = false;
	
	// Counter that is decremented faster if there are more entities in the vicinity. If it reaches negative we toggle the light.
	...
	public void update() {
		...
		if (world.isRemote) {
			// We only do something on the client so we don't have to bother ablout client-server communication.
			// The effect that we want to have (blinking the block) is client-side as well.
			updateCounter();
			counter -= lastCount * 3;
			if (counter <= 0) {
				lit = !lit;
				counter = 400;  // This is 20 seconds. Rate increases if more mobs are near
