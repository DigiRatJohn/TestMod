...
public class DataBlock extends Block implements ITileEntityProvider {
	...
	public DataBlock() {
		super(Material.ROCK);
		this.setUnlocalizedName(References.MODID + ".datablock");
		this.setRegistryName("datablock");		
	}
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
		if (!world.isRemote) {
			...
			if (side == state.getValue(FACING)) {
				...
				TextComponentTranslation component = new TextComponentTranslation("message.testmod.counter_par", counter);
	...
	public int getMetaFromState(IBlockState state) {
		// Since we only allow horizontal rotation we need only 2 bits for facing. North, South, West, East start at index 2 so we have subtract 2 here.
